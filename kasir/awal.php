...
if (isset($_POST['simpan'])) {
  // Cek apakah produk sudah ada di transaksi_temp
  $kdproduk = $_POST['kdproduk'];
  $select_existing = mysqli_query($koneksi, "SELECT * FROM transaksi_temp WHERE kdproduk='$kdproduk'");
  $existing_row = mysqli_fetch_assoc($select_existing);

  if ($existing_row) {
    // Produk sudah ada, perbarui jumlah beli
    $new_jumlah_beli = $existing_row['jumlah_beli'] + 1;
    $update_query = "UPDATE transaksi_temp SET jumlah_beli=$new_jumlah_beli WHERE kdproduk='$kdproduk'";
    mysqli_query($koneksi, $update_query);
  } else {
    // Produk belum ada, tambahkan ke transaksi_temp
    insert_transaksi();  // Pastikan fungsi ini sudah didefinisikan
  }
}

// Setelah mendapatkan $existing_row
if ($existing_row) {
  // Produk sudah ada, perbarui jumlah beli
  $new_jumlah_beli = $existing_row['jumlah_beli'] + 1;

  // Pengecekan stok
  $stok_produk = $existing_row['stok']; // Ganti dengan kolom yang sesuai
  if ($new_jumlah_beli > $stok_produk) {
    // Stok tidak mencukupi, berikan pesan kesalahan
    echo "Stok produk tidak mencukupi!";
  } else {
    // Stok mencukupi, lakukan pembaruan
    $update_query = "UPDATE transaksi_temp SET jumlah_beli=$new_jumlah_beli WHERE kdproduk='$kdproduk'";
    mysqli_query($koneksi, $update_query);
  }
}
              ...
              <div class="col-sm-12">
                <div class="well">
                  ...
                  if (!empty(isset($_POST['bayar_submit']))) {
                    ...
                    // Pastikan $total dan $bayar sudah didefinisikan sebelumnya
